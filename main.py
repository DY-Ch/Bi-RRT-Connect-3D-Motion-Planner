...
# Added a vectorized collision-checking function
def verify_path_collision(path, boundary, blocks):
    """
    Use vectorized Slab Method to verify whether the entire path collides
    """
    if len(path) < 2:
        return False

    env_min = boundary[0, 0:3]
    env_max = boundary[0, 3:6]

    for i in range(len(path) - 1):
        p1 = path[i]
        p2 = path[i + 1]

        # 1. Check whether the segment endpoints are outside the environment boundary
        if (
            np.any(p1 < env_min)
...
def draw_map(boundary, blocks, start, goal):
    ...
    hg = ax.plot(
        goal[:, 0], goal[:, 1], goal[:, 2], "go", markersize=7, markeredgecolor="k"
    )
...
def animate_dynamic_target(
    ax, goal, paths, times, goal1_stay, goal_other_stay, slowdown=5
):
    ...
    while goal_idx < len(goal) or path_idx < len(paths):
        ...
        if path_idx < len(paths) and sim_time >= path_event_times[path_idx]:
            ...
            current_path_handle = ax.plot(
                path[:, 0], path[:, 1], path[:, 2], "r-", linewidth=2
            )
            path_idx += 1
...
def runtest(mapfile, start, goal, verbose=True, dynamic_target=False):
    ...
    # Default: Bi-Directional RRT-Connect
    MP = Planner.MyPlanner(
        boundary, blocks
    )  # TODO: replace this with your own planner implementation

    # Optional: Uncomment the line below to test the Goal-biased RRT,
    # which will fail in E2_maze due to bug traps

    # MP = Planner.GoalBiasedPlanner(boundary, blocks)
    ...
    if dynamic_target:
        for p in paths:
            if verify_path_collision(p, boundary, blocks):
                collision = True
                break
    else:
        collision = verify_path_collision(path, boundary, blocks)
    ...
    if dynamic_target:
        ...
        for i, path in enumerate(paths):
            ...
            pathlength.append(
                np.sum(np.sqrt(np.sum(np.diff(path, axis=0) ** 2, axis=1)))
            )
...
def test_single_cube(verbose=True, dynamic_target=False):
    ...
    success, pathlength = runtest(
        "./maps/E4_single_cube.txt", start, goal, verbose, dynamic_target
    )
...
def test_maze(verbose=True, dynamic_target=False):
    ...
    success, pathlength = runtest(
        "./maps/E2_maze.txt", start, goal, verbose, dynamic_target
    )
    print("Success: %r" % success)
    print("Path length: %d" % pathlength)
    print("\n")
...
def test_window(verbose=True, dynamic_target=True):
    ...
    success, pathlength = runtest(
        "./maps/E6_window.txt", start, goal, verbose, dynamic_target
    )
    print("Success: %r" % success)
...
def test_tower(verbose=True, dynamic_target=False):
    ...
    success, pathlength = runtest(
        "./maps/E5_tower.txt", start, goal, verbose, dynamic_target
    )
    print("Success: %r" % success)
    print("Path length: %d" % pathlength)
    print("\n")
...
def test_flappy_bird(verbose=True, dynamic_target=False):
    ...
    success, pathlength = runtest(
        "./maps/E1_flappy_bird.txt", start, goal, verbose, dynamic_target
    )
    print("Success: %r" % success)
    print("Path length: %d" % pathlength)
    print("\n")
...
def test_room(verbose=True, dynamic_target=True):
    ...
    success, pathlength = runtest(
        "./maps/E7_room.txt", start, goal, verbose, dynamic_target
    )
    print("Success: %r" % success)
    print("Path length: %s" % pathlength)
    print("\n")
...
def test_monza(verbose=True, dynamic_target=False):
    ...
    success, pathlength = runtest(
        "./maps/E3_monza.txt", start, goal, verbose, dynamic_target
    )
    print("Success: %r" % success)
    print("Path length: %d" % pathlength)
    print("\n")
